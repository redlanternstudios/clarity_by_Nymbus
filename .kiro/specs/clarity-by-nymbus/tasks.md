...
- [x] 8. Case queue (`/servicing/cases`)
  ...
  - _BUGS FOUND AND FIXED 2026-08-01 (flagged by automated code review on the merge, addressed same day): (1) Escalate previously committed immediately with no destination confirmation, bypassing Requirement 2.3's "confirm before completing" — now opens an inline confirm panel with a destination selector before mutating state. (2) Change Status used a fixed cycle array that didn't include the terminal states Escalated/Closed — advancing status on a terminal case returned `indexOf === -1` and silently reset it to "Routing review," reopening a closed/escalated case. Now blocked with an explicit message telling the user to reopen the case first. (3) The Status filter dropdown had no `onChange` — selecting a status did nothing. Now wired to actually filter the table, including the new terminal statuses._
...
- [x] 17. Fix Case Queue action-menu bugs found by automated review
  - _Requirements: 2.3, 2.5_
  - _RESOLVED 2026-08-01: PR #9 merged with three unresolved P2 review comments from an automated code reviewer. Fixed same day: Escalate now confirms a destination before committing (Req 2.3); Change Status no longer silently reopens Escalated/Closed cases via a broken `indexOf` cycle; the Status filter dropdown is now wired and actually filters the table. See task 8 for full detail._
